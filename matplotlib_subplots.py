...
def example_plot(ax):
    ax.plot(np.random.randn(100), color=[x[0] for x in np.random.rand(3,1).tolist()])
...
plt.tight_layout()  # 自动控制subplots的大小，防止overlap
...
ax1 = plt.subplot(grid[0, 1:])  # 把(0，1)和(0，2)上的图拼起来变成一个
...
noise = pd.read_csv('2012_NYC_Noise_Complaints.csv', parse_dates=['Created Date'])
...
for date in fav_dates.items():
    ax.text(date[0], total_complaints.loc[date[0]], date[1])  # 在给定坐标处放置文字
# =====================================
...
for date in fav_dates.items():
    ...
    ax.annotate(date[1], xy=(xposition, yposition),
               ...
               arrowprops=dict(arrowstyle="->"))  # 复杂的放置文字和标注的方法
# =====================================
